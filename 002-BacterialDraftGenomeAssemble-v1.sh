...
forward=$(echo $1 | cut -d "." -f1)
reverse=$(echo $2 | cut -d "." -f1)
workpath=$PWD
...
java -jar $trimmomatic PE -phred33 \
 ...
 $forward"clean.fq.gz" output_forward_unpaired.fq.gz \
 $reverse"clean.fq.gz" output_reverse_unpaired.fq.gz \
 ILLUMINACLIP:TruSeq3-PE.fa:2:30:10 LEADING:3 \
 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
rm -rf output_forward_unpaired.fq.gz output_reverse_unpaired.fq.gz
mkdir rawReads
mv $forward".fq.gz" rawReads/
mv $reverse".fq.gz" rawReads/
gzip -d $forward"clean.fq.gz"
gzip -d $reverse"clean.fq.gz"
...
do
	spades.py -t 16 -k $kmer --careful --only-assembler -1 $workpath"/"$forward"clean.fq" -2 $workpath"/"$reverse"clean.fq" -o "kmer_"$kmer
done
cd ..
...
echo "PE= pe 350 50 " $workpath"/"$forward"clean.fq" $workpath"/"$reverse"clean.fq" >> config.file
echo '''
END
PARAMETERS
EXTEND_JUMP_READS=0
